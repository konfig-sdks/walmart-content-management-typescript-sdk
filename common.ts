...
import { readableStreamToString, WalmartError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new WalmartError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new WalmartError(e, e.response?.data)
                }
                if (innerError instanceof WalmartError) {
                    // Got "WalmartError" from the above try block
                    throw innerError;
                }
